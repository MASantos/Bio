...
#define MAX_PROCESSORS 60

static FIRST_LAST_RES res_proc[MAX_PROCESSORS];
static pthread_t thread_id[MAX_PROCESSORS];
static void *thread_result;
static int status;

void* phase_E(void *arg);


/* LJW end */

static int g_number_residue;
static int** g_dead_pair;

static ELEM_PTR* rotamer_list;

void Dead_End_Parallel(number_residue,dead_single,dead_pair,gap,n_processors)
...

{
  ...
  rotamer_list = (ELEM_PTR*)calloc(number_residue+1,sizeof(ELEM_PTR));

  if (verbose) printf("\nminimizing with gap %lf and %d processors\n",
      gap,n_processors);

  assert(n_processors < MAX_PROCESSORS);

  g_number_residue = number_residue;
  g_dead_pair = dead_pair;
   ...
   *****************************************************************/
    ...
    ****************************************************************/
	...
	if(number_cycles==MAX_CYCLE) {
      ...
      if(verbose) for(loop=0;loop<number_cycles;loop++) printf("\nCycle A %3d started with %5d sidechains",loop+1,survivors[loop]);
      fflush(0);
	    ...
	    for(rotamer_r_p=rotamer_list[residue_i];rotamer_r_p;rotamer_r_p = rotamer_r_p->next){
		  ...
		  if(!dead_pair[rotamer_r_id][rotamer_s_id]){
			...
			x += min;
		      ...
		      /* if(dead_pair[rotamer_r_id][rotamer_s_id]) break; */
		    }
		    if(x>best_of_worst_energy+gap){
  ...
  n_over_2 = number_residue / 2;

  span = ceil((double)n_over_2 / n_processors);

  printf("\n");
  for (proc = 0, curr_res = 1; curr_res <= n_over_2; ++proc) {
    res_proc[proc].first = curr_res;
    curr_res += span;
    if (curr_res - 1 > n_over_2) curr_res = n_over_2 + 1;
    res_proc[proc].last = curr_res - 1;
    res_proc[proc].do_pair_II = do_pair_II;
    res_proc[proc].proc = proc;
    res_proc[proc].gap = gap;
    if (verbose)
      printf("starting proc %d with %d to %d\n",proc,res_proc[proc].first,res_proc[proc].last);
    status = pthread_create(&thread_id[proc],NULL,phase_E,&res_proc[proc]);
    if (status != 0) {
      printf("ERROR: cannot create thread %d, aborting",proc);
      exit(1);
    }
  }

  do_pair_II = FALSE;
  for (--proc; proc >= 0; --proc) {
    status = pthread_join(thread_id[proc], &thread_result);
    if (status != 0) {
      printf("ERROR: cannot join thread %d, aborting",proc);
      exit(1);
    }
    do_pair_II |= res_proc[proc].do_pair_II;
  }
...
void process_residue(int residue_i,void* arg);
...
void* phase_E(void *arg) {
  ...
  int first_res = ((FIRST_LAST_RES*)arg)->first;
  int last_res = ((FIRST_LAST_RES*)arg)->last;

    n_over_2 = g_number_residue/2;

    printf("\nfrom %d to %d and %d to %d\n",first_res,last_res,
      (2*n_over_2+1) - last_res,(2*n_over_2+1) - first_res);

#ifdef DEBUG
    printf("\nDEBUG* Entry to Dead_End Phase E from %d to %d and %d to %d",first_res,last_res,
      (2*n_over_2+1) - last_res,(2*n_over_2+1) - first_res);
    fflush(0);
#endif
   ...
   *****************************************************************/
    ...
    ****************************************************************/
    ...
    for (residue_i = (2*n_over_2+1) - last_res; residue_i <= (2*n_over_2+1) - first_res; residue_i++)
      process_residue(residue_i,arg);

    for (residue_i = first_res; residue_i <= last_res; residue_i++)
      process_residue(residue_i,arg);

    /* for (residue_i = (2*n_over_2+1) - first_res; residue_i >= (2*n_over_2+1) - last_res; residue_i--)
      process_residue(residue_i,arg);

    for (residue_i = last_res; residue_i >= first_res; residue_i--)
      process_residue(residue_i,arg); */
...
void process_residue(int residue_i,void* arg) {
  ...
  int do_pair_II = ((FIRST_LAST_RES*)arg)->do_pair_II;
  double gap = ((FIRST_LAST_RES*)arg)->gap;
  ...
  double x, y, min;
      ...
      for(residue_j=residue_i+1;residue_j<=g_number_residue;residue_j++){

	if(alive_rotamer[residue_j]>1||alive_rotamer[residue_i]>1) {
	  
	/* printf("proc %d res %d %d\n",((FIRST_LAST_RES*)arg)->proc,residue_i,residue_j); */
	  ...
	  for(rotamer_r_p=rotamer_list[residue_i];rotamer_r_p;rotamer_r_p = rotamer_r_p->next){
  	      ...
  	      for(rotamer_s_p=rotamer_list[residue_j];rotamer_s_p;rotamer_s_p = rotamer_s_p->next){
		if(!g_dead_pair[rotamer_r_p->id][rotamer_s_p->id]){
			...
			if(!g_dead_pair[rotamer_t_p->id][rotamer_u_p->id]){
  
			  x  = Reduced_Energies[rotamer_r_p->id][rotamer_s_p->id] - Reduced_Energies[rotamer_t_p->id][rotamer_u_p->id]; 

			  /*  loop thru all other residues for summation */
			  for(residue_k=1;residue_k<=g_number_residue;residue_k++) {
			    if(residue_i!=residue_k&&residue_j!=residue_k ) {
				...
				if(!g_dead_pair[rotamer_s_p->id][rotamer_v_p->id]&!g_dead_pair[rotamer_r_p->id][rotamer_v_p->id]){
			  ...
			  if(x>gap) {
			    g_dead_pair[rotamer_r_p->id][rotamer_s_p->id]=TRUE;
			    g_dead_pair[rotamer_s_p->id][rotamer_r_p->id]=TRUE;
    ...
    fflush(0);
		      ...
		      if(g_dead_pair[rotamer_r_p->id][rotamer_s_p->id]) break;
		    }
		    if(g_dead_pair[rotamer_r_p->id][rotamer_s_p->id]) break;
		  }		    
		}
    ...
    ((FIRST_LAST_RES*)arg)->do_pair_II = do_pair_II;
