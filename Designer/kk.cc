...
public:
	...
	ostream& printDimerGraph(ostream& os);
	...
	float weight(int rot1, int rot2) const {
		...
		if (!_pair_flags[_position[rot1]][_position[rot2]])
			return -((_single_energies[rot1]+_single_energies[rot2])/(_colors.size()-1));
...
void Weights::computeMaximum(float below_maximum, int heuristic_cycles) {
	...
	for (int pos = 0; pos < size_pos(); ++pos){
	...
	for (; heuristic_cycles; --heuristic_cycles) {
		...
		for (int pos = 0; pos < size_pos(); ++pos)
			state[pos] = rotamers(pos).front()+ (int)(drand48()*rotamers(pos).size());
		...
		while (improvement && cycles--) {  // improvement loop
			...
			improvement = false;
			for (int i = 0; i < size_pos(); ++i) {  // loop over positions, but traversing randomly
				int& position = seq_walk[i];
				int best_rot = -1;
				#ifdef DEBUG
				cerr<<"cycle "<<heuristic_cycles<<" "<<cycles<<" "<<i<<" "<<state[position]<<" "<<rotamers(position).front()<<" "<<rotamers(position).back()<<endl;
				#endif
				float max_weight = -LARGE_FLOAT;
				// let's mutate a single position to all possible rot's
				for (int rot = rotamers(position).front();  // loop over rotamers
						rot <= rotamers(position).back(); ++rot) {
					float weight_sum = 0;
					...
					for (int pos2 = 0; pos2 < size_pos(); ++pos2)
						weight_sum += weight(rot,state[pos2]);
...
void Weights::collectBestNodes(bool below_flag, float below_maximum) {
	...
	for (int pos = 0; pos < size_pos(); ++pos) {
		for (int pos2 = pos+1; pos2 < size_pos(); ++pos2)
			total_weight += weight(state[pos],state[pos2]);
	}
...
ostream& Weights::printDimerGraph(ostream& os) {
	int size_pos2 = size_pos();
	int size_rot2 = size_rot();
	int mono_size_pos = int(size_pos2/2);
	if (2*mono_size_pos != size_pos2){
		cerr<<"Error: size_pos is not an even number, do we have a dimer?\n";
		exit(1);
	}
	vector< pair<int,int> > old_rot; // it gives the original rotamer number (cumulative) at each monomer
	cout<<mono_size_pos<<endl;
	int rot = 0;
	for (int position = 0; position < mono_size_pos; ++position){
		int position2 = position + mono_size_pos;
		for (int i=(rotamers(position)).front();i<=(rotamers(position)).back();++i){
			for (int j=(rotamers(position2)).front();j<=(rotamers(position2)).back();++j){
				...
				old_rot.push_back(pair<int,int>(i,j));
	...
	cerr<<"new_size_rot= "<<new_size_rot<<" old size_rot= "<<size_rot2<<endl;
		...
		for (int column = 0; column <= row; ++column) {
			// we have four terms
			float dimer_pair_energy = (weight((old_rot[row]).first,(old_rot[column]).first)-((_single_energies[(old_rot[row]).first]+_single_energies[(old_rot[column]).first])/(size_pos2-1)))+
				(weight((old_rot[row]).second,(old_rot[column]).second) -((_single_energies[(old_rot[row]).second]+_single_energies[(old_rot[column]).second])/(size_pos2-1)))+
				(weight((old_rot[row]).first,(old_rot[column]).second) -((_single_energies[(old_rot[row]).first]+_single_energies[(old_rot[column]).second])/(size_pos2-1)))+
				(weight((old_rot[row]).second,(old_rot[column]).first)-((_single_energies[(old_rot[row]).second]+_single_energies[(old_rot[column]).first])/(size_pos2-1)));
			float opt_dimer_pair_energy = dimer_pair_energy +((_dimer_single_energies[row]+_dimer_single_energies[column])/(mono_size_pos-1));
			if (_position[(old_rot[row]).first]==_position[(old_rot[column]).first])
				opt_dimer_pair_energy = 0;
			cout<<0-opt_dimer_pair_energy<<" ";
                }
		cout<<endl;
	}
	return os;
...
int main(int argc, char* argv[]) {
	cerr<<"gra2dimer created by Alfonso Jaramillo. Version "<<__DATE__<<endl;
	if (argc < 2) {
                cout << "usage: gra2dimer file.sing.dmp file.rta numer_of_processes" << endl;
                exit(1);
        }
	long what_time;
	(void)time(&what_time);
	srand48(what_time);  // remove for debugging
	...
	exit(0);
	//cerr<<"weight= "<<weights.weight(0,3)<<" "<<weights.weight(50,51)<<endl;
	/*
	// output .gra file (to check)
	cout<<size_pos<<endl;
	int rot = 0;
	for (int color = 0; color < size_pos; ++color){
		for (int i=(weights.rotamers(color)).front();i<=(weights.rotamers(color)).back();++i)
			++rot;
		cout<<rot<<" ";
	}
	cout<<endl<<endl<<rot<<endl<<endl<<endl;
        for (int row = 0; row < size_rot; ++row) {
		for (int column = 0; column <= row; ++column) {
			cout<<0-weights.weight(column,row)<<" ";
                }
		cout<<endl;
	}
	*/
