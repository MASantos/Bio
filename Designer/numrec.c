...
/* allocate an unsigned char matrix with subscript range v[nrl..nrh][ncl..nch] */
{
	long i, nrow=nrh-nrl+1,ncol=nch-ncl+1;
	char **v;

	/* allocate pointers to rows */
	v=(char **) malloc((size_t)((nrow+NR_END)*sizeof(char*)));
	if (!v) nrerror("allocation failure 1 in matrix()");
	v += NR_END;
	v -= nrl;


	/* allocate rows and set pointers to them */
	v[nrl]=(char *) malloc((size_t)((nrow*ncol+NR_END)*sizeof(char)));
	if (!v[nrl]) nrerror("allocation failure 2 in matrix()");
	v[nrl] += NR_END;
	v[nrl] -= ncl;

	for(i=nrl+1;i<=nrh;i++) v[i]=v[i-1]+ncol;
...
/* allocate a double 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
	...
	if (!t) nrerror("allocation failure 1 in df3tensor()");
	t += NR_END;
	t -= nrl;

	/* allocate pointers to rows and set pointers to them */
	t[nrl]=(double **) malloc((size_t)((nrow*ncol+NR_END)*sizeof(double*)));
	if (!t[nrl]) nrerror("allocation failure 2 in df3tensor()");
	t[nrl] += NR_END;
	t[nrl] -= ncl;

	/* allocate rows and set pointers to them */
	t[nrl][ncl]=(double *) malloc((size_t)((nrow*ncol*ndep+NR_END)*sizeof(double)));
	if (!t[nrl][ncl]) nrerror("allocation failure 3 in df3tensor()");
	t[nrl][ncl] += NR_END;
	t[nrl][ncl] -= ndl;
